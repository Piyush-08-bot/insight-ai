...
export async function checkOllamaStatus(): Promise<{
  ...
  try {
    ...
    if (response.ok) {
      ...
      const qwen = models.find((m: { name: string }) => m.name.includes('qwen'));
...
export function runPythonDirect(args: string[]): ChildProcess {
...
export function runAnalyze(
  ...
  if (options.persistDir) args.push('--persist-dir', options.persistDir);
  if (options.fileTypes) args.push('--file-types', options.fileTypes);
  if (options.embedding) args.push('--embedding', options.embedding);
